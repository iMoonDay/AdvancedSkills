...
data class SkillModifier(
    ...
    class Serializer : KSerializer<SkillModifier> {
        ...
        override fun deserialize(decoder: Decoder): SkillModifier =
            decoder.decodeStructure(descriptor) {
                ...
                var rarity: Skill.Rarity? = null
                var time: Int? = null
                while (true) {
                    when (val index = decodeElementIndex(descriptor)) {
                        0 -> cooldown = decodeIntElement(descriptor, 0)
                        1 -> rarity = Skill.Rarity.fromId(decodeStringElement(descriptor, 1))
    ...
    companion object {
        ...
        fun fromNbt(nbt: NbtCompound): SkillModifier {
            ...
            val rarity = if (nbt.contains("rarity")) Skill.Rarity.entries[nbt.getInt("rarity")] else null
