...
class DashSkill : Skill(
    ...
    override fun use(user: ServerPlayerEntity): UseResult = user.run {
        stopFallFlying()
        val multiplier = getDoubleParam(PARAM_DASH_FORCE, user, DEFAULT_DASH_FORCE)
        ...
        UseResult.success(sound = getSoundEventParam(PARAM_DASH_SOUND, DEFAULT_DASH_SOUND.get()))
    }
